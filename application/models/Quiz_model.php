...
Class Quiz_model extends CI_Model
{
 ...
 function get_questions($qids){
 	...
 	//J DEL CURA: correction de la requêtpour du SQL SERVER, l'expression "ORDER BY FIELD" valable uniquement en MySql est remplacée par la clause "CASE WHEN "	
	 if($qids=="")
	 {
	 	$qids=0;
	 	$query=$this->db->query("select * from savsoft_qbank join savsoft_category on savsoft_category.cid=savsoft_qbank.cid join savsoft_level on savsoft_level.lid=savsoft_qbank.lid
	 			where savsoft_qbank.qid in ($qids) order by savsoft_qbank.qid");
	 }
	 else
	 {
	 	$bqids=explode(",",$qids);
	 	$queryOrderBy = " order by case savsoft_qbank.qid ";
	 	
	 	foreach($bqids as $k => $qval)
	 	{
	 		$queryOrderBy = "$queryOrderBy when $qval then $k ";
	 	}
	 	$queryOrderBy = "$queryOrderBy end";
	 	$query=$this->db->query("select * from savsoft_qbank join savsoft_category on savsoft_category.cid=savsoft_qbank.cid join savsoft_level on savsoft_level.lid=savsoft_qbank.lid
	 		where savsoft_qbank.qid in ($qids) $queryOrderBy");
	 }
	 return $query->result_array();
/*
	 if($cid!='0'){
...
*/
 ...
 function get_options($qids){
 	...
 	{
 		$qids=0;
 		$query=$this->db->query("select * from savsoft_options where qid in ($qids) order by savsoft_options.qid");
 	}
 	else
 	{
 		$oqids=explode(",",$qids);
 		$queryOrderBy = " order by case savsoft_options.qid ";
 		
 		foreach($oqids as $k => $qval)
 		{
 			$queryOrderBy = "$queryOrderBy when $qval then $k ";
 		}
 		$queryOrderBy = "$queryOrderBy end";
 		$query=$this->db->query("select * from savsoft_options where qid in ($qids) $queryOrderBy");
 	}
 	return $query->result_array();
...
$noq=$quiz['noq'];	
	...
	//J DEL CURA: correction de la requêtpour du SQL SERVER, l'expression "ORDER BY FIELD" valable uniquement en MySql est remplacée par la clause "CASE WHEN "	
	$queryOrderBy = " ORDER BY CASE qid ";
	...
	$queryOrderBy = "$queryOrderBy end";
	$query=$this->db->query("select * from savsoft_qbank join savsoft_category on savsoft_category.cid=savsoft_qbank.cid where qid in ($wqids) $queryOrderBy");
...
function saved_answers($rid){
 ...
 function assign_score($rid,$qno,$score){
	...
	$percentage_obtained=round(($marks/$quiz['noq'])*100,2);
	 ...
	 foreach($_POST['answer'] as $ak => $answer){
		 ...
		 if($_POST['question_type'][$ak] == '1' || $_POST['question_type'][$ak] == '2'){
			 ...
			 $marks=0;
				...
				if($attempted==1){
					/*J DEL CURA : correction cas où la somme des points par réponse ne donne pas exactement 1
					Règle de gestion liée au calcul des résultats pour des questions à choix multiple */
					$marks = round($marks,1);
					if($marks==1){
